...
// ============================================================
// ensureListeningCoherence — semantic-mismatch fix for P1-P4
// After AI generation + other normalizers, always override
// fields from authoritative mock data so image/transcript/options/
// answer are guaranteed consistent.
// ============================================================
export function ensureListeningCoherence(questions: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  ...
  return questions.map(q => {
    ...
    if (part === 1 && type === 'listening') {
      const d = PART1_DATA[p1Index % PART1_DATA.length];
      q['image'] = d.image;
      q['options'] = [...d.options];
      q['answer'] = d.answer;
      p1Index++;
    ...
    if (part === 2 && type === 'listening') {
      const d = PART2_DATA[p2Index % PART2_DATA.length];
    ...
    if (part === 3 && type === 'listening') {
      const conv = MOCK_PART3_CONVERSATIONS[p3ConvIndex % MOCK_PART3_CONVERSATIONS.length];
      const qData = conv.questions[p3QIndex % conv.questions.length];
      q['transcript'] = conv.transcript;
      q['options'] = [...qData.options];
      q['answer'] = qData.answer;
      if (q['question'] !== qData.question) {
    ...
    if (part === 4 && type === 'listening') {
      const talk = MOCK_PART4_TALKS[p4TalkIndex % MOCK_PART4_TALKS.length];
      const qData = talk.questions[p4QIndex % talk.questions.length];
      q['transcript'] = talk.transcript;
      q['options'] = [...qData.options];
      q['answer'] = qData.answer;
      if (q['question'] !== qData.question) {
